    ...
    public boolean handleChat(Player player, String chat) {
        if(step == 0) {
            ...
            if (chat.matches("\\d+:\\d+:\\d+")) {
                quantity = chat;
                player.sendMessage(MyChatColor.GREEN + "Enter price (in diamonds)");
                step += 1;
                return true;
                ...
                player.sendMessage(MyChatColor.RED + "The format of your quantity string was incorrect.");
                player.sendMessage(MyChatColor.GREEN + "Set quantity (in format shulker:stack:num)");
                player.sendMessage(MyChatColor.GRAY + "Or type \"nil\" to cancel.");
                return true;
            }
            ...
            if (chat.matches("-?\\d+")) {
                ...
                try {
                    ...
                    if (shopRepo.addItemToShop(item, shopKey)) {
                        player.sendMessage(MyChatColor.GOLD + "Item added successfully!");
                    } else {
                        player.sendMessage(MyChatColor.RED + "Something went wrong while adding item.");
                    }
                    processHandler.discontinueProcessOfPlayer(this, uuid);
                    processHandler.discontinueProcessOfShop(this, shopKey);
                    return true;
                } catch (NumberFormatException e) {
                    player.sendMessage(MyChatColor.RED + "The format of your price was incorrect.");
                    player.sendMessage(MyChatColor.GREEN + "Enter price (in diamonds)");
                    player.sendMessage(MyChatColor.GRAY + "Or type \"nil\" to cancel.");
                    return true;
                }
                ...
                cancel();
                player.sendMessage(MyChatColor.GRAY + "Cancelled item addition");
                return true;
            }  else {
                player.sendMessage(MyChatColor.RED + "The format of your price was incorrect.");
                player.sendMessage(MyChatColor.GREEN + "Enter price (in diamonds)");
